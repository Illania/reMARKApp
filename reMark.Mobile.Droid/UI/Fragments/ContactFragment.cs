...
using reMark.Mobile.Droid.Ui.Views.ContactViews;
    ...
    public class ContactFragment : BaseFragment
    {
        ...
        public override void OnCreateOptionsMenu(IMenu menu, MenuInflater inflater)
        {
            ...
            if (ServerConfig.SystemSettings.ContactsModuleInfo.WorktrayEnabled ?? true)
                menu.AddMenuItem(Menu.None, MenuItemActions.CopyToWorktray, MenuItemActions.CopyToWorktray, Resource.String.copy_to_worktray, this);

            menu.AddMenuItem(Menu.None, MenuItemActions.CopyToFolder, MenuItemActions.CopyToFolder, Resource.String.copy_to_folder, this);

            if (folder?.InternalType == FolderInternalType.FilterView || folder?.InternalType == FolderInternalType.Static || folder?.InternalType == FolderInternalType.Worktray)
                menu.AddMenuItem(Menu.None, MenuItemActions.MoveToFolder, MenuItemActions.MoveToFolder, Resource.String.move_to_folder, this);
            menu.AddMenuItem(Menu.None, MenuItemActions.Categories, MenuItemActions.Categories, Resource.String.categories, this);

            if (contact != null)
                menu.AddMenuItem(Menu.None, MenuItemActions.Comments, MenuItemActions.Comments, Resource.String.comments, this);

            menu.AddMenuItem(Menu.None, MenuItemActions.Actions, MenuItemActions.Actions, Resource.String.history, this);
            menu.AddMenuItem(Menu.None, MenuItemActions.Links, MenuItemActions.Links, Resource.String.overview, this);

            if (folder?.InternalType == FolderInternalType.FilterView || folder?.InternalType == FolderInternalType.Static || folder?.InternalType == FolderInternalType.Worktray)
                menu.AddMenuItem(Menu.None, MenuItemActions.DeleteFromFolder, MenuItemActions.DeleteFromFolder, Resource.String.delete_from_folder, this);

            if (ServerConfig.SystemSettings.ContactsModuleInfo.Permissions.DeleteAllowed)
                menu.AddMenuItem(Menu.None, MenuItemActions.Delete, MenuItemActions.Delete, Resource.String.delete, this);

            if (ServerConfig.SystemSettings.ContactsModuleInfo.Permissions.EditAllowed)
                menu.AddMenuItem(Menu.None, MenuItemActions.Edit, MenuItemActions.Edit, Resource.String.edit, this);
        }
        ...
        async void AddChildrenContact()
        {
            ...
            var index = await Dialogs.ShowListDialog(Context, Resource.String.edit_contact_children_dialog_title, values.Select(v => GetString(UIResource.ContactTypeResourceId(v))).ToArray(), true);
            if (index >= 0)

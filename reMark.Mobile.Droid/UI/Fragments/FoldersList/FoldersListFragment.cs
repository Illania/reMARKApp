...
namespace reMark.Mobile.Droid.Ui.Fragments
{
    public class FoldersListFragment : BaseFragment, ActionMode.ICallback, IMenuItemOnActionExpandListener, SearchView.IOnQueryTextListener
    {
        ...
        async void CreateContact()
        {
            ...
            var index = await Dialogs.ShowListDialog(Context, Resource.String.edit_contact_dialog_title, values.Select(v => GetString(UIResource.ContactTypeResourceId(v))).ToArray(), true);
            if (index >= 0)

...
namespace Mark5.Mobile.Droid.Ui.Activities
{
    ...
    public class ContactEmailAddressesActivity : BaseAppCompatActivity
    {
        ...
        public const string RecipientResultKey = "RecipientResult_7638a4cd-f12f-4e8a-8862-98fd9fa208cc";
        public const int RecipientRequestCode = 777;
        ...
        {
            var intent = new Intent(context, typeof(ContactEmailAddressesActivity));
        ...
        protected override void OnCreate(Bundle savedInstanceState)
        {
            ...
            if (savedInstanceState == null)
            {
                ...
                var (cf, tag) = ContactEmailAddressesFragment.NewInstance(folderId, folder, contactId, contactPreview);
                var ft = SupportFragmentManager.BeginTransaction();
                ft.Replace(Resource.Id.fragment_container, cf, tag);
                ft.Commit();

                CommonConfig.Logger.Info($"Created {nameof(ContactEmailAddressesActivity)}");
            }
            else
            {
                CommonConfig.Logger.Info($"Restored {nameof(ContactEmailAddressesActivity)}");
            }
        }
        protected override void OnActivityResult(int requestCode, Result resultCode, Intent data)
        {
            if (requestCode == RecipientRequestCode && resultCode == Result.Ok && data.HasExtra(LinkedEmailListActivity.RecipientResultKey))
            {
                var recipientString = data.GetStringExtra(LinkedEmailListActivity.RecipientResultKey);

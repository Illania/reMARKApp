        ...
        async void SetFolderFavouriteStatusForSelection(bool isFolderFavorite)
        {
            ...
            if (PlatformConfig.Preferences.SyncFavoritesEnabled && CommonConfig.Reachability.IsReachable)
            {
                try {
                    if (isFolderFavorite)
                    {
                        await Managers.FoldersManager.AddServiceFavoriteFoldersAsync(selectedFolders, selectedFolders.First().Module);
                    } 
                    else 
                    {
                        await Managers.FoldersManager.RemoveServiceFavoriteFoldersAsync(selectedFolders, selectedFolders.First().Module);
                    }
                }
                catch
                {

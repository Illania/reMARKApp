...
using IFileSystem = reMark.Mobile.Common.Storage.AppFileStorage.Interface.IFileSystem;
...
namespace reMark.Mobile.Common.Storage.AppFileStorage
{
    ...
    public class AppFileSystem : IFileSystem
    {
        ...
        public IFolder LocalStorage
        {
            get
            {
                var localAppData = (DeviceInfo.Platform == DevicePlatform.Android)
                    ? Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments)
                    : Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments), "..", "Library");

                return new AppFileSystemFolder(localAppData);
            }
        }
        ...
        public async Task<IFile> GetFileFromPathAsync(string path, CancellationToken cancellationToken)
        {
            ...
            if (File.Exists(path))
            {
                return new AppFileSystemFile(path);
            }
        ...
        public async Task<IFolder> GetFolderFromPathAsync(string path, CancellationToken cancellationToken)
        {
            ...
            {
                return new AppFileSystemFolder(path, true);
            }

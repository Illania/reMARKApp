...
namespace reMark.Mobile.Common.Storage.AppFileStorage
{
    ...
    public class AppFileSystemFile : IFile
    {
        ...
        public AppFileSystemFile(string path)
        {
        ...
        public async Task<Stream> OpenAsync(Enum.FileAccess fileAccess, CancellationToken cancellationToken)
        {
            await AwaitExtensions.SwitchOffMainThreadAsync(cancellationToken);

            if (fileAccess == Enum.FileAccess.Read)
            {
                return File.OpenRead(Path);
            }
            else if (fileAccess == Enum.FileAccess.ReadAndWrite)
            {
        ...
        public async Task RenameAsync(string newName, NameCollisionOption collisionOption, CancellationToken cancellationToken)
        {
            ...
            await MoveAsync(System.IO.Path.Combine(System.IO.Path.GetDirectoryName(_path), newName), collisionOption, cancellationToken);
        }

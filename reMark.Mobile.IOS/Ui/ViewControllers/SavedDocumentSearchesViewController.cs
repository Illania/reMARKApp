...
{
    public class SavedDocumentSearchesViewController: AbstractTableViewController, IUISearchResultsUpdating
    {
        readonly TaskCompletionSource<SavedDocumentsSearch> tcs = new TaskCompletionSource<SavedDocumentsSearch>();
        public Task<SavedDocumentsSearch> Result => tcs.Task;
        ...
        public void RowSelected(SavedDocumentsSearch sds)
        { 
            tcs.SetResult(((DataSource)TableView.Source).SelectedSearch);
            DismissViewController(true, null);
        }
        ...
        static bool MatchesQuery(SavedDocumentsSearch sds, string query)
        {
        ...
        async void DeleteSavedSearch(SavedDocumentsSearch savedDocumentsSearch)
        {
            ...
            try
            {
                ...
                await Managers.SearchManager.DeleteSavedSearchAsync(savedDocumentsSearch.Id);
                dismissAction();
            ...
            {
                CommonConfig.Logger.Error($"Failed to delete saved document search [Id={savedDocumentsSearch.Id}] "
                                             , ex.InnerException);
        ...
        class DataSource : UITableViewSource
        {
            ...
            readonly WeakReference<SavedDocumentSearchesViewController> viewControllerWeakReference;
            readonly WeakReference<UITableView> tableViewWeakReference;

            bool loading = true;
            public List<SavedDocumentsSearch> Items = new();

            public DataSource(SavedDocumentSearchesViewController viewController, UITableView tableView)
            {
                viewControllerWeakReference = viewController.Wrap();
                tableViewWeakReference = tableView.Wrap();
            }

            public SavedDocumentsSearch SelectedSearch
            {
                get
                {
            ...
            public void RemoveSavedSearch(SavedDocumentsSearch savedDocumentsSearch)
            {
            ...
            public override UITableViewRowAction[] EditActionsForRow(UITableView tableView, NSIndexPath indexPath)
            {
                ...
                var savedDocumentsSearch = Items[indexPath.Row];


                var deleteAction = UITableViewRowAction.Create(UITableViewRowActionStyle.Destructive,
                                                                Localization.GetString("delete"),
                                                                (a, ip) => viewControllerWeakReference.Unwrap()?.DeleteSavedSearch(savedDocumentsSearch));     
                deleteAction.BackgroundColor = Theme.DarkerBlue;
                actions.Add(deleteAction);
                return actions.ToArray();
            }

            public void SetItems(List<SavedDocumentsSearch> destinations)
            {
                loading = false;

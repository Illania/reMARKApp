    ...
    public class ComposeDocumentViewController : AbstractWebViewController, IUIAdaptivePresentationControllerDelegate
    {
        ...
        void InitNavigationBar()
        {
            ...
            if (ServerConfig.SystemSettings?.SystemInfo?.DelaySendAvailable == true)
            {
                ...
                sendButton.TitleLabel.Font = Theme.DefaultBoldFont.CustomFont();
        ...
        async Task LoadTemplate()
        {
            ...
            switch (PlatformConfig.Preferences.UseTemplate)
            {
                ...
                case Preferences.TemplateUsageMode.AlwaysAsk:
                    ...
                    var result = await Dialogs.ShowListActionSheetAsync(this, templateListStrings, headerStackView);
                    switch (result)
                    {
                        case 0:
                            await InsertDefaultTemplate();
        ...
        void RecipientView_SearchRequested(object sender, string initialSearchString)
        {
            ...
            if (suggestionsListView == null)
            {
                ...
                {
                    suggestionsListView.TopAnchor.ConstraintEqualTo(View.SafeAreaLayoutGuide.TopAnchor),
                    suggestionsListView.LeftAnchor.ConstraintEqualTo(View.SafeAreaLayoutGuide.LeftAnchor),
                    suggestionsListView.RightAnchor.ConstraintEqualTo(View.SafeAreaLayoutGuide.RightAnchor),
                    suggestionsListView.BottomAnchor.ConstraintEqualTo(View.SafeAreaLayoutGuide.BottomAnchor)
                });

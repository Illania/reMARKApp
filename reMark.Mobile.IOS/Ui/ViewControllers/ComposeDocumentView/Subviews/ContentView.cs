    ...
    public class ContentView : ComposeDocumentSubView, IWKNavigationDelegate, IUIGestureRecognizerDelegate, IWKScriptMessageHandler, IUIScrollViewDelegate
    {
        ...
        public override async Task InitializeView()
        {
            ...
            if (DocumentCreationModeFlag == DocumentCreationModeFlag.Edit ||
                (DocumentCreationModeFlag == DocumentCreationModeFlag.New && CopyToNewOption == CopyToNewOption.Attachments))
            {
        ...
        async Task<string> GetBodyWithHeader(string content, ContentType contentType)
        {
            if (contentType == ContentType.Html)
            {
                ...
                var htmlDocument = await htmlParser.ParseDocumentAsync(content);
                var body = htmlDocument.Body;
                var parsedHeader = await htmlParser.ParseDocumentAsync(htmlHeader);
        ...
        async Task<(string content, string preview)> RetreiveContentAndPreview()
        {
            ...
            var newContentParsed = await htmlParser.ParseDocumentAsync(newContentString) ;
            ...
            var oldContentParsed = await htmlParser.ParseDocumentAsync(oldContentString);
            ...
            {
                var newContentProcessedParsed = await htmlParser.ParseDocumentAsync(newContentProcessedString);
                var oldContentProcessedParsed = await htmlParser.ParseDocumentAsync(oldContentProcessed);
        ...
        async Task SetWebContentPart(string parentElementClass, ContentType contentType, string content)
        {
            ...
            var htmlParser = new HtmlParser();
            var currentHtmlDocument = await htmlParser.ParseDocumentAsync(currentContent);
        ...
        static async Task<string> ProcessInsertedContent(HtmlParser htmlParser, ContentType contentToInsertType, string contentToInsert)
        {
            if (contentToInsertType == ContentType.Html)
            {
                ...
                var htmlDocument = await htmlParser.ParseDocumentAsync(inlinedContentToInsert);
                return htmlDocument.Body.InnerHtml;
            }

            if (contentToInsertType == ContentType.PlainText)
            {
                var htmlDocument = await htmlParser.ParseDocumentAsync("<div><pre>" + contentToInsert + "</pre></div>");
                return htmlDocument.Body.InnerHtml;
            }

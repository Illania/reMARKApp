...
{
    public class SavedShortcodeSearchesViewController: AbstractTableViewController, IUISearchResultsUpdating
    {
        readonly TaskCompletionSource<SavedShortcodesSearch> tcs = new TaskCompletionSource<SavedShortcodesSearch>();
        public Task<SavedShortcodesSearch> Result => tcs.Task;
        ...
        public void RowSelected(SavedShortcodesSearch sds)
        { 
            tcs.SetResult(((DataSource)TableView.Source).SelectedSearch);
            DismissViewController(true, null);
        }
        ...
        static bool MatchesQuery(SavedShortcodesSearch sds, string query)
        {
        ...
        async void DeleteSavedSearch(SavedShortcodesSearch savedShortcodesSearch)
        {
            ...
            try
            {
                ...
                await Managers.SearchManager.DeleteSavedSearchAsync(savedShortcodesSearch.Id);
                dismissAction();

                ((DataSource)TableView.Source).RemoveSavedSearch(savedShortcodesSearch);
            ...
            {
                CommonConfig.Logger.Error($"Failed to delete saved document search [Id={savedShortcodesSearch.Id}] "
                                             , ex.InnerException);
        ...
        class DataSource : UITableViewSource
        {
            ...
            readonly WeakReference<SavedShortcodeSearchesViewController> viewControllerWeakReference;
            readonly WeakReference<UITableView> tableViewWeakReference;

            bool loading = true;
            public List<SavedShortcodesSearch> Items = new();

            public DataSource(SavedShortcodeSearchesViewController viewController, UITableView tableView)
            {
                viewControllerWeakReference = viewController.Wrap();
                tableViewWeakReference = tableView.Wrap();
            }

            public SavedShortcodesSearch SelectedSearch
            {
                get
                {
            ...
            public void RemoveSavedSearch(SavedShortcodesSearch savedShortcodesSearch)
            {
                var position = Items.FindIndex(c => c.Id == savedShortcodesSearch.Id);
                if (position >= 0)
                {
            ...
            public override UITableViewRowAction[] EditActionsForRow(UITableView tableView, NSIndexPath indexPath)
            {
                ...
                var SavedShortcodesSearch = Items[indexPath.Row];


                var deleteAction = UITableViewRowAction.Create(UITableViewRowActionStyle.Destructive,
                                                                Localization.GetString("delete"),
                                                                (a, ip) => viewControllerWeakReference.Unwrap()?.DeleteSavedSearch(SavedShortcodesSearch));     
                deleteAction.BackgroundColor = Theme.DarkerBlue;
                actions.Add(deleteAction);
                return actions.ToArray();
            }

            public void SetItems(List<SavedShortcodesSearch> destinations)
            {
                loading = false;

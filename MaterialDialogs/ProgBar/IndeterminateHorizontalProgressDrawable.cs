...
{
    public class IndeterminateHorizontalProgressDrawable : BaseIndeterminateProgressDrawable, IShowBackgroundDrawable
    {
        static int ProgressIntrinsicHeightDP = 4;
        static int PaddedIntrinsicHeightDP = 16;
        static RectF RectBound = new RectF(-180, -1, 180, 1);
        static RectF RectPaddedBound = new RectF(-180, -4, 180, 4);
        static RectF RectProgress = new RectF(-144, -1, 144, 1);
        static RectTransformX Rect1TransformX = new RectTransformX(-522.6f, 0.1f);
        static RectTransformX Rect2TransformX = new RectTransformX(-197.6f, 0.1f);
            ...
            : base(context)
		{
            ...
            Animators = new Animator[] {
                ProgBar.Animators.CreateIndeterminateHorizontalRect1(mRect1TransformX),
                ProgBar.Animators.CreateIndeterminateHorizontalRect2(mRect2TransformX),
        ...
        protected override void OnDraw(Canvas canvas, int width, int height, Paint paint)
        {
            ...
            if (showBackground)
            {
                paint.Alpha = Java.Lang.Math.Round(Alpha * backgroundAlpha);
